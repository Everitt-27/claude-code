...
function shadow(x,y,rx){
  ctx.fillStyle='rgba(14,12,9,.30)';
  ctx.beginPath();ctx.ellipse(x+rx*.5,y+1,rx*1.4,rx*.4,.26,0,TAU);ctx.fill();
}
...
function rig(x,y,o){
  ...
  }else if(pose==='lurch'){ /* the dead reach for the living */
    T1=pol(S1.x,S1.y,face,9*s);T1.y-=1.2*s;
    T2=pol(S2.x,S2.y,face,7.5*s);T2.y+=.4*s;
  ...
  /* tattered leather tassets over the hips */
  if(pose!=='ride'){
    ctx.fillStyle=shade(pants,1.3);
    for(const tx2 of[-2.4,0,2.4]){
      const swx=o.mv?Math.sin(ph+tx2)*.5*s:0;
      ctx.beginPath();
      ctx.moveTo((tx2-1.15)*s,hipY+.1*s);
      ctx.lineTo((tx2+1.15)*s,hipY+.1*s);
      ctx.lineTo((tx2+.95)*s+swx,hipY+3.9*s);
      ctx.lineTo((tx2+.15)*s+swx,hipY+3.2*s);
      ctx.lineTo((tx2-.95)*s+swx,hipY+3.9*s);
      ctx.closePath();ctx.fill();
    }
    ctx.fillStyle='rgba(0,0,0,.2)';
    ctx.beginPath();ctx.roundRect(-3.5*s,hipY+.1*s,7*s,1*s,.5*s);ctx.fill();
  }
  /* belt */
  ctx.fillStyle='rgba(24,18,13,.6)';
  ctx.beginPath();ctx.roundRect(-3.7*s,hipY-.4*s,7.4*s,1.7*s,.8*s);ctx.fill();
  dotc(0,hipY+.45*s,.7*s,'#a8865a');
  ...
  /* fur mantle bunched across the shoulders (heavy sets) */
  if(arm&&arm.fur){
    for(let fi=-3;fi<=3;fi++){
      const fx2=fi*1.5*s+lx,fy3=shY+ly-1.1*s+((fi&1)?.4*s:0);
      dotc(fx2,fy3,(1.7-Math.abs(fi)*.13)*s,(fi&1)?'#7b756a':'#918a7d');
    }
    dotc(lx-3.4*s,shY+ly-.3*s,1.2*s,'#6a655c');
    dotc(lx+3.4*s,shY+ly-.3*s,1.2*s,'#6a655c');
  }
  ...
  if(o.paint){ctx.fillStyle=o.paint;
    ctx.fillRect(lx+fx+.7*s,hy+ly-.8*s+fy2,1.2*s,3*s);}
...
const NPCLOOK={
  elder:{body:'#847d6f',hair:'#cfd6da',skin:'#d2ab84'},
  boro:{body:'#5f4232',apron:'#42352b',hair:'#3a2d22',skin:'#c69a70'},
  pip:{body:'#a05a42',hair:'#6e4228',size:.72},
  sella:{body:'#6a5578',hat:'#b08a48'},
  wren:{body:'#46607e',hair:'#7c6142',glasses:1},
  holt:{body:'#55503f',hood:'#7a5f42'},
  rhoa:{body:'#525f6e',hair:'#241c16',skin:'#b8834f',paint:'rgba(42,54,72,.6)'},
  zef:{body:'#96603c',hat:'#d8ccae'},
  juno:{body:'#5d7a4a',hair:'#241c16',size:.72}};
...
function drawEnt(e){
  ...
  switch(e.t){
    case'npc':{
      ...
      const look=e.look||NPCLOOK[e.key]||{body:'#7a6a5a'};
      const o={face:e.vface,anim:e.anim,mv,mvv:hyp(e.vx,e.vy),
        mdir:mv?Math.atan2(e.vy,e.vx):e.vface};
      const guard=e.job==='guard'||e.job==='captain';
      if(guard){
        o.arm=ARM.mail;o.wMain=jobTool('guard');
        if(e.st==='windup'||e.st==='swing'){
          o.pose='swing';o.poseT=e.st==='windup'?.2:.75;o.face=e.face;o.swingDir=1;}
      }else if(e.working){
        if(e.job==='smith'||e.job==='farmer'||e.job==='wood'){
          o.pose='over';o.poseT=clamp((e.wkT||0)/(e.job==='smith'?1.1:1.5),0,1);
          o.wMain=jobTool(e.job);
        }else if(e.job==='herb'){o.pose='over';o.poseT=.92;}
        else if(e.job==='fisher'&&e.fishSpot){
          o.pose='fish';
          const rl=rotD(e.fishSpot.x-e.x,e.fishSpot.y-e.y);
          o.rodLine={x:e.x+rl.x,y:e.y+rl.y};
        }
      }
      if(e.job==='hunter')o.backBow=1;
      rig(e.x,e.y,Object.assign({},look,o));
      break;}
    case'ally':{
    ...
    case'blob':{
      ...
      shadow(e.x,e.y+6,8);
      ctx.fillStyle=fl?'#fff':'#688e55';
      ctx.beginPath();ctx.ellipse(e.x,e.y,9*(2-sq)*.9,8*sq,0,0,TAU);ctx.fill();
      ctx.fillStyle='#5a7048';ctx.beginPath();ctx.ellipse(e.x,e.y+3,7*(2-sq)*.9,4*sq,0,0,TAU);ctx.fill();
    ...
    case'deer':{
      ...
      const c=fl?'#fff':'#b1905f',gt=mv?e.anim*7:e.anim*1.6;
    ...
    case'bandit':case'brute':{
      ...
      rig(e.x,e.y,{face:(wu||e.st==='swing')?e.face:e.vface,anim:e.anim,mv,
        ...
        hood:wu?'#a05436':'#7a5c46',skin:'#b98a5f',
        paint:e.t==='brute'?'rgba(140,42,30,.55)':null,
    ...
    case'zombie':{
      shadow(e.x,e.y+9,7);
      if(e.vface===undefined)e.vface=e.face;
      e.vface+=angDiff(e.vface,e.face)*Math.min(1,(G.ldt||.016)*6);
      ctx.save();
      if(e.rise>0){
        const k=clamp(e.rise,0,1);
        ctx.beginPath();ctx.rect(e.x-27,e.y-48,54,58);ctx.clip();
        dotc(e.x,e.y+8,10,'#4a4034');
        dotc(e.x-6,e.y+10,4,'#3a332a');dotc(e.x+7,e.y+9,3.4,'#3a332a');
        ctx.translate(0,k*26);
      }
      rig(e.x,e.y,{face:e.vface,anim:e.anim*.55,mv,mvv:hyp(e.vx,e.vy),
        mdir:mv?Math.atan2(e.vy,e.vx):e.vface,
        body:fl?'#fff':'#4c523d',c2:'#3a3f30',pants:'#3f3a2c',
        skin:'#8aa072',hair:'#33301f',paint:'rgba(52,74,44,.6)',
        pose:'lurch'});
      ctx.restore();
      break;}
    case'skel':{
  ...
  if(e.elite&&!e.dead&&e.hp>0){
    ...
    ctx.restore();
    ctx.font='700 8px Georgia,serif';ctx.textAlign='center';
    ctx.fillStyle=afc;
...
function drawPlayer(){
  ...
  rig(p.x,p.y,{
    ...
    pose,poseT:pT,swingDir,scarf:1,paint:'rgba(52,36,26,.8)',
...
function drawProp(p){
  ...
  switch(p.t){
    ...
    case'field':{
      ...
      for(let r2=0;r2<3;r2++){
        ...
        for(let i2=0;i2<5;i2++){
          ...
          else{dotc(gx,gy,2.2,'#7c9256');dotc(gx,gy-2.6,1.6,owned?'#e8b04a':'#8ca35f');}
        }
      }
    ...
    case'bigtree':case'ftree':{
      ...
      dotc(p.x+sw,p.y-16*s,13*s,'#3d5537');
      dotc(p.x-4*s+sw,p.y-20*s,9*s,'#4d6a45');
    ...
    case'bush':{
      ...
      shadow(p.x,p.y+5,8);
      dotc(p.x,p.y,8,'#4e623c');dotc(p.x-4,p.y-3,5,'#5a7048');dotc(p.x+4,p.y-2,5.4,'#5a7048');
      break;}
    case'boulder':{
    ...
    case'well':{
      shadow(p.x,p.y+6,11);
      ctx.fillStyle='#7b7466';ctx.beginPath();ctx.ellipse(p.x,p.y+2,11,7,0,0,TAU);ctx.fill();
      ctx.fillStyle='#5d5750';ctx.beginPath();ctx.ellipse(p.x,p.y,10,6.4,0,0,TAU);ctx.fill();
      ctx.fillStyle='#1c1814';ctx.beginPath();ctx.ellipse(p.x,p.y,7,4.4,0,0,TAU);ctx.fill();
      rr(p.x-9,p.y-20,2.6,20,1,'#5f4232');rr(p.x+6.4,p.y-20,2.6,20,1,'#5f4232');
      ctx.fillStyle='#4a3524';
      ctx.beginPath();ctx.moveTo(p.x-12,p.y-18);ctx.lineTo(p.x,p.y-27);ctx.lineTo(p.x+12,p.y-18);ctx.closePath();ctx.fill();
      ctx.strokeStyle='#3a2d22';ctx.lineWidth=1;
      ctx.beginPath();ctx.moveTo(p.x,p.y-17);ctx.lineTo(p.x,p.y-8);ctx.stroke();
      rr(p.x-2,p.y-8,4,4,1,'#8a744a');
      break;}
    case'lamp':{
      rr(p.x-1.6,p.y-26,3.2,28,1.4,'#3a2f22');
      rr(p.x-4,p.y-31,8,7,2,'#1c1814');
      const lit=G.dayT>.66||G.dayT<.07;
      if(lit){ctx.save();ctx.globalCompositeOperation='lighter';
        dotc(p.x,p.y-27,10+Math.sin(G.vt*7+p.x)*1.6,'rgba(232,170,90,.30)');ctx.restore();
        dotc(p.x,p.y-27.5,2.2,'#ffd66e');}
      else dotc(p.x,p.y-27.5,2,'#6a5a44');
      break;}
    case'crate':{
      shadow(p.x,p.y+4,8);
      rr(p.x-7,p.y-7,14,13,1.5,'#6a4a34');
      ctx.strokeStyle='#4a3524';ctx.lineWidth=1;
      ctx.strokeRect(p.x-7,p.y-7,14,13);
      ctx.beginPath();ctx.moveTo(p.x-7,p.y-7);ctx.lineTo(p.x+7,p.y+6);
      ctx.moveTo(p.x+7,p.y-7);ctx.lineTo(p.x-7,p.y+6);ctx.stroke();
      break;}
    case'bench':{
      rr(p.x-11,p.y-4,22,3.4,1.4,'#6a4a34');
      rr(p.x-9,p.y-1,2.4,5,1,'#4a3524');rr(p.x+7,p.y-1,2.4,5,1,'#4a3524');
      break;}
    case'shroomp':{
      if(op)break;
...
function drawFt(dt){
  ...
  for(const f of G.ft){
    ...
    ctx.globalAlpha=a;
    ctx.font='700 '+((f.big?13.5:10.5)*K).toFixed(1)+'px Georgia,serif';
...
function drawTree(o){
  ...
  blob(cx,cy,11,'#3d5537');
  blob(cx-7,cy+2,7.5,'#3d5537');
  blob(cx+7,cy+2,7.5,'#3d5537');
  blob(cx-3,cy-6,8,'#4d6a45');
  blob(cx+4,cy-4,7.5,'#587552');
